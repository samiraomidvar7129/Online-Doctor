export const displayDoctors = (doctors) => {
  let doctorsContainer = document.querySelector("#doctorsList .swiper-wrapper");
  for (let doctor in doctors)
    doctors[doctor].forEach((doctor) => {
...
export const displayComments = (comments) => {
  let commentsContainer = document.querySelector("#comments .swiper-wrapper");
  for (let comment in comments)
    comments[comment].forEach((item) => {
...
export const displayBlogs = (blogs) => {
  let blogsContainer = document.querySelector("#blogs .swiper-wrapper");
  for (let blog in blogs)
    blogs[blog].forEach((blog) => {
